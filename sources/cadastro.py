...
def cadastroEscuderia(connection, window):
    ...
    window.geometry(f"{width}x{350}")
    window.configure(bg="#2C3E50")

    fWindow = Frame(window, bg="#2C3E50")
    fWindow.pack(fill="both", expand=True, padx=20, pady=20)

    lHeader = cria_label(fWindow, "Cadastro de Escuderias", 24, "bold")
    lHeader.pack(padx=10, pady=10)

    fForm = Frame(fWindow, bg="#2C3E50")
    fForm.pack(padx=10, pady=5)

    lRef = cria_label(fForm, "ConstructorRef:", 12, "normal")
    lRef.grid(row=0, column=0, pady=5, sticky="w")
    eRef = cria_entry(fForm, "Insira o constructorRef", 10)
    eRef.grid(row=0, column=1, padx=5, pady=5)

    lnome = cria_label(fForm, "Nome:", 12, "normal")
    lnome.grid(row=1, column=0, pady=5, sticky="w")
    enome = cria_entry(fForm, "Insira o nome", 10)
